globalThis.MainSettings = class MainSettings {
  ...
  constructor(script, alternative) {
    this.#config = MainSettings.hasOwnProperty(script) ? MainSettings[script] : typeof alternative === "string" && MainSettings.hasOwnProperty(alternative) ? MainSettings[alternative] : null;
    return this;
  }
  merge(settings) {
 ...
 */ const deserialize = (str = "", repair = false) => {
  try {
    const parsed = JSON.parse(repair ? (() => {
      ...
      return first === -1 || last === -1 || last <= first ? "{}" : `{${str.slice(first, last + 1)}}`;
    })() : str);
 ...
 */ const deindicate = (card = {}) => {
  if (typeof card.title !== "string") {
    ...
    card.title = card.title.slice(card.title.indexOf("​") + 1).replaceAll("​", "").trim();
  }
  return;
};
 ...
 */ const simplify = (str = "") => {
  str = str.replace(/[#*~•·∙⋅]+/g, "").replace(/  +/g, " ").replace(/ ?\n ?/g, "\n").replaceAll("…", "...").replace(/([sS])(['‘’‛])[sS]/g, (_, s, q) => `${s}${q}`).replace(/[–−‒]/g, "-").replace(/(?<=\S) [-—] (?=\S)/g, "—");
  // Convert one lone em-dash to a semicolon if appropriate
    return (str.match(/—/g) || []).length === 1 && !str.includes(";") && !str.endsWith("—") && !str.startsWith("—") ? str.replace("—", "; ") : str;
};
 ...
 */ const formatKey = (k = "") => k.trim().split(/\s/, 1)[0].replace(/[.'`´‘’]+/g, "").replace(/[^a-z0-9A-Z_]/g, "_").replace(/([a-z0-9])([A-Z])/g, (_, a, b) => `${a}_${b.toLowerCase()}`).toLowerCase().replace(/([a-z])([0-9])/g, (_, a, b) => `${a}_${b}`).replace(/([0-9])([a-z])/g, (_, a, b) => `${a}_${b}`).replace(/__+/g, "_").replace(/(?:^_|_$)/g, "");
 ...
 */ const ownership = (name = "") => `${name}${name.endsWith("'") || name.endsWith("'s") ? "" : name.toLowerCase().endsWith("s") ? "'" : "'s"}`
/**
 ...
 */ const joinMind = (mind = [], unlabeled = false) => mind.map(([label, key, thought]) => `${unlabeled ? "" : `[${label}] `}(${key}: \`${thought}\`)`).join("\n");
 ...
 */ class Config {
  ...
  static get(pending = new Set) {
    ...
    const u = "qm`x/`hetofdno/bnl.qsnghmd.Tlcs`Pthmm".replace(/./g, c => String.fromCharCode(c.charCodeAt() ^ 1));
     ...
     */    const template = {
      ...
      entry: [ {
        ...
        message: "Enable QuillScript:",
        ...factory("allow", S.IS_QS_ENABLED_BY_DEFAULT)
      }, {
        message: "Show detailed guide:",
        builder: (cfg = {}, hook = "", info = {}) => ` ${hook === "context" || Number.isInteger(info.maxChars) ? config.guide ?? cfg.setter?.(false) : false}`,
        ...
        message: "Adventure in 1st, 2nd, or 3rd person:",
        ...factory("pov", S.FIRST_SECOND_OR_THIRD_PERSON_POV, {
          lower: 1,
          upper: 3,
          suffix: () => [ "st", "nd", "rd" ][config.pov - 1] ?? ""
        })
      }, {
        message: "Max brain size relative to story context:",
        ...factory("percent", S.PERCENTAGE_OF_RECENT_STORY_USED_FOR_BRAINS, {
          lower: 1,
          upper: 95,
          suffix: "%"
        })
      }, {
        message: "Recent turns searched for name triggers:",
        ...factory("distance", S.NUMBER_OF_ACTIONS_TO_LOOK_BACK_FOR_TRIGGERS, {
          lower: 1,
          upper: 250
        })
      }, {
        message: "Visual indicator of current NPC triggers:",
        builder: (cfg = {}) => ` "${config.indicator ?? cfg.setter?.(S.ACTIVE_CHARACTERS_VISUAL_INDICATOR_SYMBOL)}"`,
        setter: (value = null, fallible = false) => typeof value === "string" ? config.indicator = value.replace(/["\u200B-\u200D]+/g, "").trim() : fallible ? null : config.indicator = fallback.indicator
      }, {
        message: "Thought formation chance per turn:",
        ...factory("chance", S.THOUGHT_FORMATION_CHANCE_PER_TURN, {
          lower: 0,
          upper: 100,
          suffix: "%"
        })
      }, {
        message: "Half thought chance for Do/Say/Story:",
        ...factory("half", S.IS_THOUGHT_CHANCE_HALF_FOR_DO_SAY_STORY)
      }, {
        message: "Brain card notes store brains as JSON:",
        ...factory("json", S.IS_JSON_FORMAT_USED_FOR_BRAIN_CARD_NOTES)
      }, {
        message: "Enable debug mode to see model tasks:",
        ...factory("debug", S.IS_DEBUG_MODE_ENABLED_BY_DEFAULT)
      }, {
        message: "Pin this config card near the top:",
        ...factory("pin", S.IS_CONFIG_CARD_PINNED_BY_DEFAULT)
      }, {
      ...
      description: [ {
        ...
        builder: (cfg = {}) => [ "", "", ...config.agents ?? cfg.setter?.(S.IMPORTANT_SCENARIO_CHARACTERS), "" ].join("\n"),
        setter: (value = null, fallible = false) => {
          ...
          // Clean, deduplicate, and remove empties
                    return config.agents = [ ...new Set(config.agents.map(agent => cleanAgentLocal(agent)).filter(agent => agent !== "")) ];
        }
      } ]
    };
      ...
      if (!card || typeof card !== "object" || Array.isArray(card)) {
        ...
        while (t < target.length && c < current.length) {
          ...
          // Allow for insertions, deletions, or substitutions
                    mistakes++;
          current[c + 1] === target[t] ? c++ : current[c] === target[t + 1] ? t++ : (t++, 
          c++);
        }
     ...
     */    const build = (source = [], delimiter = "\n\n") => source.map(cfg => `> ${cfg.message}${cfg.builder?.(cfg) ?? ""}`).join(delimiter);
    if (config.card === null) {
      // If no config card exists, create one and recurse
      addStoryCard(u, build(template.entry, "\n"), template.type, template.title, build(template.description, "\n\n"));
    ...
    // Abomination :3
        [ "entry", "description" ].map(source => [ source, typeof config.card[source] === "string" ? Object.fromEntries(config.card[source].split(/\s*>[\s>]*/).filter(block => block.includes(":")).map(block => block.split(/\s*:[\s:]*/, 2)).map(pair => [ simplifyLocal(pair[0]), pair[1].trimEnd() ])) : {} ]).forEach(([source, extractive]) => template[source].forEach(cfg => 
 ...
 */ class Agent {
   ...
   */  get card() {
     ...
     */    const buildCard = (name = this.name) => addStoryCard(JSON.stringify({
      agent: this.name
    }), (() => {
      ...
      return `// initialized @ ${match ? `${match[3]}-${match[1]}-${match[2]} ${match[4]}` : time.toISOString().replace("T", " ").slice(0, 16)} UTC`;
    })(), "Brain", name, JSON.stringify({}), 
    // Thank you Mavrick
    {
      returnCard: true
    });
     ...
     */    const isAgent = (card = {}) => typeof card.keys === "string" && card.keys.includes('"agent"') && deserialize(card.keys).agent === this.name;
   ...
   */  get metadata() {
    ...
    // Validate and normalize the percent value
      ...
      // Uh oh
      this.#metadata.percent = typeof this.#metadata.percent === "number" && Number.isFinite(this.#metadata.percent) ? Math.min(Math.max(lower, Math.round(this.#metadata.percent)), upper) : this.#percent;
   ...
   */  get brain() {
    ...
    // Parse the brain from the card description using the simple format
      ...
      // Right of colon
      clean.slice(bisector + 1) ].map(twin => twin.replace(/(?:^[\s{},"_\\]*|[\s{},"_\\]*$)/g, ""));
      if (key !== "" && value !== "") {
 ...
 */ const prompt = new Proxy({
  ...
  get(t, p) {
    ...
    return typeof t[p] === "function" ? t[p]().trim() : t[p] && typeof t[p] === "object" ? new Proxy(t[p], this) : t[p];
  }
});
 ...
 */ function handleContextHook(config, QS, history, storyCards, text, info) {
  ...
  // (this block is blazingly fast)
    const possibilities = [];
  for (let [i, remaining] = [ history.length - 1, config.distance ]; 0 < remaining && -1 < i && possibilities.length === 0; i--) {
    ...
    // Check each agent in priority order
      ...
      // Scan for all occurrences of agentLower in lower
            for (let p = lower.indexOf(agentLower); p !== -1; p = lower.indexOf(agentLower, p + 1)) {
        // Ensure word boundaries (not a-z before or after)
        if ([ 0 < p ? lower.charCodeAt(p - 1) : 0, p + agentLower.length < lower.length ? lower.charCodeAt(p + agentLower.length) : 0 ].every(c => c < 97 || 122 < c)) {
  ...
  if (config.debug) {
    ...
    if (start !== -1) {
      ...
      text = `${text.slice(0, start + boundary.upper.length)}${text.slice(start + boundary.upper.length).replace(/\s*\([\s\S]*?\)\s*/g, "\n\n")}`;
    }
  }
  // Construct the agent instance for the triggered NPC
    const agent = new Agent(QS.agent, {
   ...
   */  const mind = (() => {
    ...
    const direction = Math.random() < .7 ? 1 : -1;
    ...
    // Parse each thought and extract label/content
      ...
      // Parse label and thought, handle malformed values
            const [label, thought] = sliceIndex === -1 ? [ unknown, value.trim() ] : [ parseInt(value.slice(0, sliceIndex), 10) || unknown, value.slice(sliceIndex + 1).trim() ];
    ...
    // Thoughts without integer labels ("[*]") are placed above (60%) or below (40%) the rest
        return Math.random() < .6 ? [ ...unknowns, ...numbered ] : [ ...numbered, ...unknowns ];
  })();
  ...
  // Check if the current turn is a retry or erase + continue following a previous task completion
    ...
    text = `${nondirective(config.player, config.pov)}${bindSelf(agent.name, mind.map(([label, key, thought]) => `- ${key}: ${thought} [${label}]`).join("\n"))}${text.trim()} `;
  } else {
     ...
     */    const [self, full] = (() => {
      ...
      // Only applies when brain is at least 800 chars
            const constrained = 800 < joined.length && agent.metadata.percent / 100 * (text.length - text.indexOf(boundary.upper) + boundary.upper.length) < joined.length;
      if (!constrained || Math.random() < .4) {
    ...
    "do", "say", "story" ].includes(getPrevAction(history)?.type) ? 200 : 100) < Math.random() ? (
    // Sometimes do nothing and emit a side effect on QS.agent
    QS.agent = " ", `${nondirective(config.player, config.pov)}${self}${text.trim()} `) : `${prompt.directive[pov]}${self}${text.trim()}${boundary.lower}${
    // Low context = simple prompt, high context = advanced prompt
    limit < 2e4 ? prompt.assign[pov] : prompt.choice[pov]}\n\n`;
  }
  // ==================== CONTEXT TRUNCATION ====================
  // Three-phase truncation to fit within AID's context limit
    ...
    const lowerIndex = upperIndex !== -1 ? text.indexOf(boundary.lower, upperIndex + boundary.upper.length) : -1;
    ...
    // Remove from left to right
      ...
      if (0 < storyLength) {
        ...
        Math.floor(storyLength * .85), 
        // Keep at least 2000 chars of recent story context
        Math.max(0, storyLength - 2e3), 
 ...
 */ function handleOutputHook(hook, config, QS, history, storyCards, text) {
  ...
  // Models sometimes forget their parentheses, the poor dears
    if (!/[()\[\]{}]/.test(text) && (/^\s*(?:del(?:et(?:e[ds]?|ing))?|for(?:get(?:s|ting)?|got(?:ten)?)|remov(?:e[ds]?|ing))(?:[\s_]*(?:key(?:_name)?|thought|memory|unwanted(?:_key)?))?[\s=:]*[a-z0-9A-Z]*_+[a-z0-9A-Z]/i.test(text) || /^\s*[a-z0-9A-Z_]+\s*=/.test(text))) {
  ...
  // Create an agent instance for the triggered NPC
    const agent = QS.agent === "" ? null : new Agent(QS.agent, {
    percent: config.percent
  });
  // Reset QS.agent
    QS.agent = "";
  ...
  // Process extracted block and queue appropriate operations
    ...
    deblock: {
      ...
      // Chars to consume along with the block
        ...
        // Just for fun, no regex :3
                return code === 32 || code === 9 || code === 10 || code === 13 || code === 39 || code === 96 || code === 180 || code === 8216 || code === 8217;
      };
    ...
    /** @returns {string|null} */    const delKey = (() => {
      // Match various forms of "delete key_name"
      const delMatch1 = str.match(/^(?:del(?:et(?:e[ds]?|ing))?|for(?:get(?:s|ting)?|got(?:ten)?)|remov(?:e[ds]?|ing))(?:[\s_]*(?:key(?:_name)?|thought|memory|unwanted(?:_key)?))?[\s=:]*([\s\S]*)$/i);
      if (!delMatch1) {
        return null;
      }
      ...
      // Try again with stricter matching
            const delMatch2 = str.match(/^(?:del(?:et(?:e[ds]?|ing))?|for(?:get(?:s|ting)?|got(?:ten)?)|remov(?:e[ds]?|ing))[\s=:]*([\s\S]*)$/i);
    ...
    // Extract and clean the key name
        const key = (() => {
      const raw = formatKey((delimiter === "=" ? rightOfColonLocal(str.split("=", 1)[0]) : str.split(":", 1)[0]).trim().replaceAll(" ", "_"));
      // If key exists in brain, use it as-is
      // Otherwise strip common prefixes/suffixes models tend to add
            return raw in agent.brain ? raw : raw.replace(/^th(?:oughts?|ink(?:ing))_(?:(?:o[nfr]|a(?:bout|nd)|with|for)_)?/, "").replace(/(?:_(?:and|or))?_th(?:oughts?|ink(?:ing))$/, "");
    })();
    if (key === "" || (60 < key.length || [ "thought", "thoughts", "think", "thinking", "any_name", "example_name" ].includes(key) || [ "any_key", "key_name", "example_key" ].some(s => key.includes(s))) && !(key in agent.brain)) {
      // Skip invalid or placeholder keys copied from the task prompts
      continue;
    }
    // ==================== VALUE EXTRACTION ====================
    // Extract and clean the value
        const value = (str.split(delimiter, 2)[1] || "").replace(/^[\s"'`«»„“”「」´‘’‟‚‛]+|[\s"'`«»„“”「」´‘’‟‚‛]+$/g, "").replace(/\s+/g, " ");
    ...
    // Extract the actual thought content
        const thought = simplify(rightOfColon(value).replaceAll("→", " ").replaceAll("\\n", "\n")).trim().split("\n", 1)[0].trimEnd();
    if (altered.has(key) || !thought.includes(" ")) {
      ...
      for (const key in brain) {
        ...
        typeof existing === "string" && last < existing.length && existing.indexOf(thought) !== -1) {
    ...
    // Queue an assign operation
      ...
      // Inject the encoding into the output text
            text = text.replace(/[\u200B-\u200D]+/g, "").replace(/^\s*/, leadingWhitespace => `${leadingWhitespace}${QS.encoding}`);
      ...
      // Determine if this is a relabel of the same thought value
            const relabel = typeof old === "string" && thought === old.slice(old.indexOf("→") + 1).trim();
      return `${relabel ? `old = ${target};\n` : ""}${target} = ${relabel ? `[${QS.label}, old${old.includes("→") ? '\n  .slice(old.indexOf("→") + 1)\n  .trim()\n' : ".trim()"}].join(" → ")` : JSON.stringify(agent.brain[key])};`;
    });
    altered.add(key);
  }
  ...
  // This removes artifacts, formatting issues, and unwanted patterns
    text = simplify(config.debug ? text : text.replaceAll("_", "")).trim().split("\n").filter(line => {
    const lower = line.toLowerCase();
    return !(
    // The nuclear option
    /(?:^|[^a-zA-Z])(?:task|output)(?:$|[^a-zA-Z])/.test(lower) || [ "STRICT", "OUTPUT", "REQUIRE", "EXACT", "TASK", "FORMAT", "QuillScript", `You are ${config.player}.` ].some(naughty => line.includes(naughty)) || lower.includes("story") && lower.includes("continu") || /^\[?\d+(?:\.?\]|\.)/.test(lower) || /^\s*user(?:$|[^a-z])/.test(lower) || /^[ -]+$/.test(lower));
  }).join("\n").trim().replace(/\n\n\n+/g, "\n\n");
  // ==================== OUTPUT FINALIZATION ====================
  ...
  // Build the JSON-like string manually for each key-value pair
    let serialized = "";
  const appendPair = config.json ? (serialized = `"${keys[0]}": ${JSON.stringify(brain[keys[0]])}`, 
  (key = "") => {
    // Format -> "key": "value",\n\n (JSON with linebreaks)
    serialized += `,\n\n"${key}": ${JSON.stringify(brain[key])}`;
    return;
  }) : (serialized = `${keys[0]}: ${brain[keys[0]]}`, (key = "") => {
 ...
 */ globalThis.QuillScript = function QuillScript(hook) {
  ...
  // Without these, QuillScript literally cannot function
    if (!globalThis.state || typeof state !== "object" || Array.isArray(state) || !globalThis.info || typeof info !== "object" || Array.isArray(info) || !Array.isArray(globalThis.storyCards) || typeof addStoryCard !== "function" || !Array.isArray(globalThis.history) || typeof text !== "string") {
